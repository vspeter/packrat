...
class DistroVersion( models.Model ):
  ...
  distro = models.CharField( max_length=DISTORY_LENGTH, choices=DISTRO_CHOICES )
  ...
  def __getattr__( self, name ):
    if name == 'package_list':
      ...
      if self.release_type == 'ci':
        qs = qs.filter( packagefile__ci_at__isnull=False, packagefile__dev_at__isnull=True, packagefile__prod_at__isnull=True ).distinct()

      elif self.release_type == 'dev':
        qs = qs.filter( packagefile__dev_at__isnull=False, packagefile__prod_at__isnull=True ).distinct()
...
class PackageFile( models.Model ):
  ...
  distro_version = models.ForeignKey( DistroVersion, editable=False )
  ...
  def checkfile( self, request_distro=None ):
    if not self.file._file: # no new file, nothing to look at, return what we curently have, just to be sane
      ...
      if extension == 'deb':
        ...
        try:
          ( package, version, release, arch ) = re.match( '(.+)-([^-]+)-([^-]+)\.(\w+)', name )
        except ValueError:
      ...
      for distroversion in DistroVersion.objects.filter( file_type=package_type ):
        full_distroversion_list.append( distroversion.pk )
        for name in distroversion.release_names.split( '\t' ):
          if name in version:
            distroversion_list.append( distroversion.pk )

      if request_distro:
        if request_distro in full_distroversion_list:
          distro = request_distro

      elif len( distroversion_list ) == 1:
        distro = distroversion_list[0]

      elif len( full_distroversion_list ) == 1:
        distro = full_distroversion_list[0]

      if not distro: # confused, punt to the caller
  ...
  def save( self, *args, **kwargs ):
    if not self.distro:
      self.checkfile()
      if not self.distro:
        raise ValidationError( 'distro not set, run checkfile manually before saving' )
  ...
  class Meta:
    unique_together = ( 'package', 'distro', 'version', 'type', 'arch' )
